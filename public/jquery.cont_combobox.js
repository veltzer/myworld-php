      ...
      jQuery.widget('ui.cont_combobox', {
        ...
        _create: function(options) {
          ...
          var select = this.element.hide(), selected = select
              .children(':selected'), value = selected.val() ?
              selected.text() : '';
          var input = jQuery('<input>').insertAfter(select).val(value)
              .autocomplete(
                  {
                    delay: 0,
                    minLength: 0,
                    source: function(request, response) {
                      var matcher = new RegExp(jQuery.ui.autocomplete
                    ...
                    change: function(event, ui) {
                      ...
                      if (!ui.item) {
                        var matcher = new RegExp('^' +
                            jQuery.ui.autocomplete.escapeRegex(jQuery(this)
                                .val()) + '$', 'i'), valid = false;
          ...
          jQuery('<button>&nbsp;</button>').attr('tabIndex', -1).attr('title',
            ...
            text: false
              ...
              'ui-corner-right ui-button-icon').click(function() {
